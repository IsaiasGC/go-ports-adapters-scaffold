...
// Si se encuentra una estructura anidada, la función se llama recursivamente para verificar los
...
func GetMissingEnvVars(configuration any) []string {
	...
	for i := 0; i < value.NumField(); i++ {
		...
		default:
			...
			if envVarValue, exists := os.LookupEnv(strings.Split(envVarTag, ",")[0]); !exists || envVarValue == "" {
				missingVars = append(missingVars, strings.Split(envVarTag, ",")[0])
			}
		}
	}
	return missingVars
