...
This project uses [swaggo/swag](https://github.com/swaggo/swag) and [echo-swagger](https://github.com/swaggo/echo-swagger) to generate and serve API documentation.
...
There you can explore and test all API endpoints interactively.

### Update generated Swagger docs

Whenever you add or modify API endpoints, regenerate the Swagger documentation:

```bash
swag init -g .\cmd\api\main.go -o .\api\
```

- `-g` specifies the main file where your Echo server is initialized.
- `-o` specifies the dir that will contain the output files.
- This will update the `api/` folder with the latest API definitions.

### Generated files

- `api/swagger.json` → OpenAPI spec in JSON format.
- `api/swagger.yaml` → OpenAPI spec in YAML format.
- `api/docs.go` → Go bindings used by echo-swagger.

⚠️ Do not edit files in api/ manually — they are auto-generated by swag init.
